...
function Load:Init()
	...
	local files = file.Find(MR:GetMapFolder().."*", "Data")

	for k,v in pairs(files) do
		load.list[v:sub(1, -5)] = MR:GetMapFolder()..v
	end

	-- Set the autoLoad command
	local value = file.Read(MR:GetAutoLoadFile(), "Data")
...
function Load:Start(ply, loadName)
	...
	-- Get the load file
	local loadFile = load.list[loadName] or MR:GetMapFolder() .. loadName .. ".txt"
...
function Load:Auto_Set(ply, loadName)
	...
	CVars:Replicate(ply, "mapret_autoload", loadName, "load", "autoloadtext", true)

	timer.Create("MapRetWaitToSave", 0.3, 1, function()
		file.Write(MR:GetAutoLoadFile(), GetConVar("mapret_autoload"):GetString())
	end)
