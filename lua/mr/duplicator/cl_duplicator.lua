...
function Duplicator:CheckForErrors(material, material2)
	-- material = { newMaterial = material }
	-- material2 = { newMaterial2 = material2 }
	...
	if MR.CL.Materials:ValidateReceived(material) == MR.Materials:GetMissing() and
	   material2 and MR.CL.Materials:ValidateReceived(material2) == MR.Materials:GetMissing() then
		Duplicator:SetErrorProgress(material)
	end
end

-- Update the duplicator progress: client
function Duplicator:SetProgress(current, total)
...
function Duplicator:SetErrorProgress(mat)
	local ply = LocalPlayer()
