...
concommand.Add("mr_cleanup", function (ply)
	...
	if not MR.Materials:IsRunningProgressiveCleanup() and MR.SV.Materials:RestoreLists(MR.SV.Ply:GetFakeHostPly()) then
...
concommand.Add("mr_rem_disp", function (ply, cmd, args)
	...
	if MR.Displacements:GetDetected()[value] then
		...
		net.Start("CL.Displacements:RestoreDetected")
			net.WriteString(value)
